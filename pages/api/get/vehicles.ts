...
export default function handler(
    ...
    if (!req.query["make"]) {
        res.status(400).json({
            status: "Missing make query parameter",
            vehicles: null,
        });
        return;
    }
    if (!req.query["model"]) {
        res.status(400).json({
            status: "Missing model query parameter",
            vehicles: null,
        });
        return;
    }
    ...
    fetch(
        `http://${process.env.NEXT_PUBLIC_BASE_URL}/api/get/makes?year=${req.query["year"]}`
    )
        .then((response) => response.json())
        .then((data) => {
            if (!data.makes.includes(req.query["make"])) {
                res.status(400).json({
                    status: "Invalid make for provided year",
                    vehicles: null,
                });
                return;
            }
    ...
    fetch(
        `http://${process.env.NEXT_PUBLIC_BASE_URL}/api/get/models?year=${req.query["year"]}&make=${req.query["make"]}`
    )
        .then((response) => response.json())
        .then((data) => {
            if (!data.models.includes(req.query["model"])) {
                res.status(400).json({
                    status: "Invalid model for provided year and make",
                    vehicles: null,
                });
                return;
            }
    ...
    fetch(
        `https://fueleconomy.gov/ws/rest/vehicle/menu/options?year=${req.query["year"]}&make=${req.query["make"]}&model=${req.query["model"]}`,
        {
